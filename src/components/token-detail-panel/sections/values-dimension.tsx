...
const UNITS: DimensionUnit[] = ['px', 'rem', 'em', '%', 'pt']

function convert(raw: string, unit: DimensionUnit): string {
  const n = parseFloat(raw)
  if (isNaN(n)) return ''
  switch (unit) {
    case 'px':  return `${n}px`
    case 'rem': return `${+(n / 16).toFixed(4)}rem`
    case 'em':  return `${+(n / 16).toFixed(4)}em`
    case '%':   return `${n}%`
    case 'pt':  return `${+(n * 0.75).toFixed(2)}pt`
  }
}

export function ValuesDimension({ modeValues }: Props) {
